...
inline int calc_f(char*, char** ,int);
int calc_maxf(char**,char**,int,int);
int calc_df(char*,char ***,int[]);
void pop_string(char ***, char **, int[],int);
float score(char**, char**, int, int, char***, int[]);
...
float score(char** word, char** doc, int m, int n, char ***save, int arr[1000])
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		if (df == 0) 
			continue;
		score += (0.5 + 0.5 * f / maxf) *
			log((float)(N) / df) / log(2.0);
	}
	return score;
}